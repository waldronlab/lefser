...

{
  ...
  contast_within_classes_or_few_per_class <- function(expr1_sub_t_df, rand_s, min_cl, ncl){
    ...
    # detect if for each class there are fewer than the minimum (min_cl) number of samples
    if (TRUE %in% c(table(cls) < min_cl)){
      return (TRUE)
    }
    # separate the randomly selected samples with replacement (cols) into a list of the two classes
    drops <- c("class")
    by_class <- lapply(seq_along(groups), function(x){cols[cols[, "class"]==groups[x],!(names(cols) %in% drops)]})

    # within each class, count unique "counts" for each feature (i.e., microorganism, column) to make sure each has no less than min_cl
  ...
  lda_fn <- function (data){
    # test 1000 samples for contrast within classes per feature and there is at least a minimum number of samples per class
