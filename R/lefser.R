...
fillPmatZmat <- function(class, subclass, relab_sub, p.threshold, method) {
    if (!nrow(relab_sub)) return(relab_sub)
    # creates a list of boolean vectors, each vector indicates
    # existence (TRUE) or absence (FALSE) of a class/sub-class combination
    combos <- apply(
        expand.grid(levels(class), levels(subclass)),
        1L,
        paste0,
        collapse = "."
    )
    ...
    whichlist <- lapply(
        setNames(nm = sort(combos)),
        function(combo) which(combo == combined)
    )
    ...
    inds <- apply(
        iters,
        1L,
        function(x) unlist(whichlist[x], use.names = FALSE)
    )
...
createUniqueValues <- function(df, class) {
    ...
    for (i in seq_along(splitdf)) {
        ...
        splitdf[[i]][] <- lapply(sdat, function(cols) {
            if (length(unique(cols)) > maxim[i]) cols else
                abs(
                    cols +
                        rnorm(
                            length(cols),
                            mean = 0,
                            sd = max(cols * 0.05, 0.01)
                        )
                )
        })
    }
...
lefser <- function(
    ...
    message(
        "The outcome variable is specified as '",
        classCol,
    ...
    scores_df <- data.frame(
        features = names(processed_sorted_scores),
